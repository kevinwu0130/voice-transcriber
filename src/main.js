...
;(() => {
  ...
  pickLang(document.getElementById('live-lang-row'), lang => {
    currentLang = lang
  ...
  function startRecording() {
    ...
    recognition.onresult = (e) => {
      ...
      resultBox.innerHTML = ''
      resultBox.appendChild(document.createTextNode(finalText))
      if (interim) {
    ...
    // Auto-restart to keep continuous recording alive
  ...
  copyBtn.addEventListener('click', () => {
    ...
    copyText(finalText); flashSuccess(copyBtn, '✅ 已複製')
  })
  ...
  clearBtn.addEventListener('click', () => {
    finalText = ''; charCount.textContent = ''
...
;(() => {
  ...
  let currentLang  = 'chinese'   // null = auto-detect
  let worker       = null
  let workerReady  = false
  let resultText   = ''

  // Single onmessage dispatcher — avoids race condition from multiple reassignments
  let messageHandler = null
  function createWorker() {
    const w = new Worker(new URL('./whisper.worker.js', import.meta.url), { type: 'module' })
    w.onmessage = ({ data }) => { if (messageHandler) messageHandler(data) }
    return w
  }

  pickLang(document.getElementById('file-lang-row'), lang => {
    // 'auto' maps to null so Whisper skips the language param (true auto-detect)
  ...
  function handleFile(file) {
    if (!file || !file.type.startsWith('audio/')) {
      alert('請選擇音訊檔案（MP3、M4A、WAV、OGG、FLAC 等）'); return
    }
    selectedFile = file
  ...
  transcribeBtn.addEventListener('click', async () => {
    ...
    // Decode audio → Float32Array at 16 kHz mono
    let audioData
    try {
    ...
    if (!workerReady) {
      ...
      const loaded = await new Promise((resolve, reject) => {
        ...
        workerReady = false; messageHandler = null; resetBtn(); return false
      })
    ...
    await new Promise((resolve) => {
      ...
      worker.postMessage({ type: 'transcribe', audio: audioData, language: currentLang })
    })
  ...
  copyBtn.addEventListener('click', () => {
    ...
    copyText(resultText); flashSuccess(copyBtn, '✅ 已複製')
  })
  saveBtn.addEventListener('click', () => {
    if (!resultText) return
